...
def classify_arg(value: object, ptr_registry: dict[int, TensorInfo]) -> ArgDescriptor:
    if isinstance(value, int) and value in ptr_registry:
        ...
        return ArgDescriptor(
            kind="tensor", buffer_id=info.buffer_id, value=None, dtype=info.dtype
        )
...
def test_observe_records_launch_with_grid_and_args(monkeypatch):
    ...
    monkeypatch.setattr("hyperonnx.replay.runner.build_ptr_registry", lambda: registry)
...
def test_observe_records_multiple_launches_in_order(monkeypatch):
    ...
    monkeypatch.setattr("hyperonnx.replay.runner.build_ptr_registry", lambda: {})
...
def _load_bundle_symbol_map(bundle_dir: Path) -> dict[str, Path]:
    ...
    return {k["symbol"]: Path(bundle_dir) / k["cubin"] for k in manifest["kernels"]}
...
def replay(
    ...
    if missing:
        raise RuntimeError(f"kernels not found in bundle {bundle_dir}: {missing}")
...
def _replay(launches, symbol_map, sample_args, ref_output, registry_before, cupy):
    ...
    all_buffer_infos: dict[int, TensorInfo] = {
        info.buffer_id: info for info in registry_before.values()
    }
    ...
    for rec in launches:
        ...
        for arg in rec.args:
            if arg.kind == "tensor":
                cbuf = ptr_to_buffer.get(arg.buffer_id) or ref_buffers.get(
                    arg.buffer_id
                )
                if cbuf is None:
